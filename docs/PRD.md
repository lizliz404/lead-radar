# PRD：Lead Radar 社媒需求信号雷达

## 1. 产品一句话

Lead Radar 是一个代码优先的社媒需求信号雷达，用来从 Reddit 等公开社区中自动发现“可能有付费意图、外包意图、强痛点或产品机会”的帖子，并生成可行动的线索报告。

---

## 2. 背景

最初的设想是用 n8n 搭建一个“AI 社媒洞察 Agent”：用户在飞书群里 @ 机器人，输入需求，系统自动抓取 Reddit 数据、清洗、分析、写入飞书多维表格，并把报告推回群里。

复盘后判断：

- “社媒需求洞察”是真需求；
- “必须使用 n8n 完成”不是需求，而是实现手段；
- 真正关键的是数据源、检索策略、信号筛选、输出可行动性；
- MVP 应该优先用代码实现一个稳定、可测试、可低成本运行的闭环。

---

## 3. 目标用户

### 3.1 第一目标用户

**Liz 自己 / 独立服务提供者 / 自动化服务操盘者**

典型需求：

- 想知道英文社区里有没有人正在寻找 n8n、Zapier、Make、自动化、内部工具相关帮助；
- 想找到潜在服务机会、内容选题、产品切口；
- 不想每天手刷多个社区；
- 愿意人工复核少量高质量线索。

### 3.2 次级用户

- Indie hacker：找小众需求和微型 SaaS 机会；
- 内容创作者：找选题、痛点、案例；
- 产品经理：监控竞品抱怨和用户反馈；
- 早期销售：找公开社区里的潜在线索。

---

## 4. 核心问题

用户当前的问题不是“没有 AI 总结”，而是：

1. 社区信息太多，手动刷效率低；
2. 搜索关键词不稳定，容易漏掉真实需求；
3. 平台搜索结果缺乏业务语义筛选；
4. LLM 直接联网搜索样本量和可控性不足；
5. 泛泛总结无法指导行动；
6. 没有持续积累历史信号，难以观察趋势。

---

## 5. 产品目标

MVP 目标：

> 每次运行后，输出一份 Top 10 以内的高信号需求报告，让用户用 10-20 分钟完成原本 1-2 小时的社区扫描。

V1 目标：

> 形成每日自动运行、飞书推送、历史去重、人工反馈的个人工作流。

V2 目标：

> 扩展为多平台、多主题、可评分迭代的需求情报系统。

---

## 6. 非目标

MVP 阶段明确不做：

- 不做大而全的“全网社媒监听”；
- 不做复杂 Web UI；
- 不做自动私信、自动销售触达；
- 不做长期囤积用户原始内容；
- 不做没有来源链接的“玄学洞察”；
- 不把 n8n 作为必要依赖；
- 不一开始引入复杂 Agent 框架。

---

## 7. MVP 范围

### 7.1 数据源

MVP 仅支持：

- Reddit 公开帖子；
- 本地 mock 数据，用于无凭证调试。

后续扩展：

- RSS；
- Hacker News；
- Product Hunt；
- GitHub Issues；
- X / 小红书 / 知乎等合规数据源。

### 7.2 输入

用户通过配置文件定义：

- 主题名称；
- 主题描述；
- 目标 subreddit 列表；
- 检索关键词；
- 强信号短语；
- 排除短语；
- 时间窗口；
- 每个来源最大抓取数；
- 输出 Top N。

### 7.3 输出

MVP 输出：

- Markdown 报告；
- SQLite 历史记录；
- 可选飞书 webhook 文本推送。

每条线索输出字段：

| 字段 | 说明 |
|---|---|
| title | 原帖标题 |
| source | 来源平台 |
| subreddit | 来源社区 |
| url | 原帖链接 |
| score | 规则评分 |
| buying_intent | strong / medium / weak / none |
| pain_summary | 痛点摘要 |
| evidence | 命中的证据短语 |
| recommended_action | 推荐行动 |
| created_at | 发布时间 |
| upvotes | 点赞数 |
| num_comments | 评论数 |

---

## 8. 核心用户故事

### Story 1：每日扫描自动化服务需求

作为自动化服务提供者，
我想每天自动扫描 n8n / automation / smallbusiness 等社区，
以便发现有人是否正在寻找自动化顾问、外包帮助或 Zapier 替代方案。

验收标准：

- 可以配置目标 subreddit 和关键词；
- 可以运行一次扫描；
- 输出 Top 10 结果；
- 每条结果有原帖链接和推荐行动。

### Story 2：快速复核报告

作为使用者，
我想在一份 Markdown 或飞书消息里看到结果，
以便快速判断今天有没有值得点开的需求。

验收标准：

- 报告中有总览；
- Top Leads 按优先级排序；
- 每条线索可追溯到原帖；
- 报告不输出泛泛建议。

### Story 3：低成本调试

作为开发者，
我想不用 Reddit 凭证也能用 mock 数据跑通流程，
以便快速验证评分、报告、推送逻辑。

验收标准：

- `--mock` 模式可运行；
- 不需要外部 API key；
- 能生成示例报告。

---

## 9. 功能需求

### 9.1 配置管理

P0：

- 支持 YAML 配置；
- 支持多个 topic；
- 支持指定 topic 运行；
- 配置缺失时报可读错误。

P1：

- 支持 per-topic 输出目录；
- 支持 per-topic 飞书 webhook；
- 支持关键词组权重。

### 9.2 数据抓取

P0：

- 支持 Reddit OAuth；
- 支持按 subreddit + keyword 搜索；
- 支持 lookback_hours 过滤；
- 支持 max_posts_per_source 限制；
- 支持去重。

P1：

- 读取 Reddit rate limit header；
- 抓取失败自动重试；
- 将抓取状态写入日志。

### 9.3 过滤与评分

P0：

- 命中 include_phrases 加分；
- 命中 exclude_phrases 降权或剔除；
- 标题和正文命中关键词加分；
- 评论数、点赞数、发布时间参与评分；
- 输出 Top N。

P1：

- 支持不同主题的评分权重；
- 支持人工反馈后调整权重；
- 支持“已看过”去重。

### 9.4 报告生成

P0：

- 生成 Markdown 报告；
- 总结本次扫描数量、候选数量、Top N 数量；
- 每条线索包含证据和行动建议。

P1：

- 加入 LLM 结构化分析；
- 加入趋势观察；
- 加入竞品/服务机会分类。

### 9.5 存储

P0：

- 使用 SQLite 保存扫描记录、帖子、线索；
- 避免重复保存同一 source_id；
- 保存 report_path。

P1：

- 支持导出 CSV；
- 支持 Supabase / Postgres；
- 支持历史趋势统计。

### 9.6 通知

P0：

- 支持飞书 webhook 推送文本摘要；
- 支持关闭通知。

P1：

- 支持飞书交互式卡片；
- 支持按钮：有用 / 无用 / 已处理；
- 支持推送失败重试。

---

## 10. 评分策略

MVP 采用规则优先评分，不依赖 LLM 判断全量数据。

### 10.1 加分项

| 信号 | 加分 |
|---|---:|
| 命中主题关键词 | +1 / 次 |
| 命中强求助短语 | +3 |
| 命中付费/外包意图 | +4 |
| 命中痛点短语 | +2 |
| 评论数较多 | +0.5 ~ +2 |
| 近 24 小时 | +1 |
| 来源社区高度相关 | +1 ~ +2 |

### 10.2 减分/剔除项

| 信号 | 处理 |
|---|---|
| 明显广告 | 降权或剔除 |
| 课程推广 | 降权或剔除 |
| 招聘帖 | 降权 |
| 过旧内容 | 剔除 |
| 无正文且标题弱相关 | 降权 |

### 10.3 购买意图分级

- strong：明确出现 willing to pay / hire / consultant / freelancer / budget / paid help；
- medium：强求助 + 业务场景明确，但未直接提钱；
- weak：有痛点但不明确寻求解决方案；
- none：普通讨论或泛泛提问。

---

## 11. 成功指标

MVP 成功不是“跑起来”，而是“值得每天看”。

| 指标 | 目标 |
|---|---:|
| 单次人工复核时间 | ≤ 20 分钟 |
| Top 10 可点击率 | ≥ 30% |
| 每周强信号线索 | ≥ 3 条 |
| 报告生成失败率 | < 10% |
| 无来源链接洞察 | 0 条 |
| 单次运行成本 | 接近 0，LLM 前置前为 0 |

---

## 12. 技术方案

### 12.1 技术栈

- Python 3.11+
- Typer：CLI
- httpx：HTTP 请求
- Pydantic：数据模型
- PyYAML：配置
- SQLite：本地存储
- Rich：终端输出

### 12.2 部署方式

MVP：

- 本地 CLI；
- GitHub Actions Cron；
- VPS Cron。

V1：

- Docker；
- Supabase / Postgres；
- 飞书群推送。

V2：

- Web dashboard；
- 多用户配置；
- 多数据源管理。

---

## 13. 风险与约束

### 13.1 数据源风险

- Reddit API 有速率限制和条款约束；
- 各平台反爬规则变化会影响可用性；
- 小红书、X 等平台的数据接入要优先考虑合规方式。

应对：

- 使用官方 API 或授权数据源；
- 低频运行；
- 不长期保存不必要原文；
- 对数据源做可插拔设计。

### 13.2 结果质量风险

- 关键词太窄会漏；
- 关键词太宽会噪音大；
- LLM 可能泛泛总结；
- 规则可能偏置。

应对：

- 固定高质量 source 白名单；
- 先规则筛选，再 LLM；
- 保留人工反馈；
- 报告必须输出证据。

### 13.3 产品膨胀风险

最容易走偏的方向：

- 一开始做复杂 UI；
- 一开始做全平台；
- 一开始做 Agent；
- 一开始做 CRM；
- 一开始做 n8n 可视化自动化。

纠偏原则：

> 先每天稳定找出 3-10 条值得点开的真实帖子，再谈系统化。

---

## 14. 里程碑

### Milestone 0：项目定义

- README
- PRD
- 技术架构
- 配置样例

### Milestone 1：本地 MVP

- mock 模式可运行；
- Reddit 抓取可运行；
- 规则评分可运行；
- Markdown 报告可生成；
- SQLite 可写入。

### Milestone 2：每日工作流

- GitHub Actions 定时运行；
- 飞书推送；
- 历史去重；
- 错误日志。

### Milestone 3：LLM 分析

- Top N 结构化分析；
- 机会分类；
- 回复建议；
- 报告质量评估。

### Milestone 4：多平台扩展

- RSS；
- Hacker News；
- Product Hunt；
- GitHub Issues；
- 合规的中文平台数据源。

---

## 15. MVP 验收清单

- [ ] `lead-radar run --mock` 能生成报告；
- [ ] 报告中每条线索有 URL；
- [ ] 配置文件可以新增 topic；
- [ ] Reddit 凭证配置后能抓真实数据；
- [ ] 同一帖子不会重复出现在结果里；
- [ ] Top N 排序符合直觉；
- [ ] 飞书 webhook 可推送摘要；
- [ ] SQLite 能保存扫描历史；
- [ ] 无 API key 时有清晰报错；
- [ ] README 足够让未来开发者接手。
